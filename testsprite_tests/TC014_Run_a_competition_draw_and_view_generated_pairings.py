...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the login page at http://localhost:6006/login and verify the login form inputs are present.
        ...
        # -> Wait briefly for SPA to finish rendering, then reload /league/fixtures to attempt to get the fixtures UI and locate the draw controls.
        await page.goto("http://localhost:6006/league/fixtures")
        try:
            await page.wait_for_load_state("domcontentloaded", timeout=5000)
        except Exception:
            pass
        
        # -> Click the 'Generate Fixtures' button (element [11204]) to run the draw and then verify that generated match pairings are displayed.
        # button "Generate Fixtures"
        elem = page.locator("xpath=/html/body/div[2]/main/div[2]/div/div/div[3]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
