...
const USERS = [
    { key: 'organizer', email: 'organizer.test@plyaz.test', fullName: 'Olivia Organizer', role: 'organizer' },
...
async function ensureUser(u) {
    ...
    const { error: pErr } = await db.from('profiles').upsert(
        { id, email: u.email, full_name: u.fullName, role: u.role, approval_status: 'approved' },
        { onConflict: 'id' },
    );
...
async function main() {
    ...
    await db.from('profiles').update({ organization_id: org.id, role: 'organizer' }).eq('id', ids.organizer);
    // referee + player belong to the org too (so org-scoped views show them)
    await db.from('profiles').update({ organization_id: org.id }).in('id', [ids.player, ids.referee]);

    const { data: category } = await db.from('categories')
        .insert({ organization_id: org.id, name: 'Senior Men', description: 'Open age', min_age: 18, max_age: 45 })
        .select().single();

    const { data: venue } = await db.from('venues')
        .insert({ organization_id: org.id, name: 'Central Stadium', address: '1 Stadium Rd', city: 'Lisbon', capacity: 5000, surface_type: 'grass' })
        .select().single();

    console.log('Creating competition…');
    const { data: comp, error: cErr } = await db.from('competitions').insert({
        organization_id: org.id, name: 'Premier Test League 2026', description: 'Seeded sample league for QA',
        type: 'league', status: 'active', season: '2026', year: 2026, category_id: category?.id,
        start_date: '2026-05-01', end_date: '2026-08-30', max_teams: 8,
    }).select().single();
    if (cErr) throw new Error(`competition: ${cErr.message}`);

    await db.from('championship_config').insert({
        competition_id: comp.id, format: 'round_robin', groups_count: 1, teams_per_group: 4,
    ...
    for (const t of teamDefs) {
        const { data, error } = await db.from('teams').insert({
            ...t, organization_id: org.id, competition_id: comp.id, manager_id: ids.organizer,
    ...
    for (let ti = 0; ti < teams.length; ti++) {
        ...
        for (let p = 0; p < 7; p++) {
            ...
            const { data, error } = await db.from('players').insert({
                team_id: teams[ti].id, organization_id: org.id, profile_id: linkProfile,
                name: linkProfile ? 'Pedro Player' : `${teamDefs[ti].short_name} Player ${p + 1}`,
                position: POSITIONS[p], jersey_number: p + 1, nationality: NATIONS[p % NATIONS.length],
                status: 'active',
            }).select().single();
    ...
    for (let m = 0; m < pairs.length; m++) {
        ...
        const day = new Date('2026-05-10'); day.setDate(day.getDate() + m * 7);
        const { data, error } = await db.from('matches').insert({
            competition_id: comp.id, organization_id: org.id,
            home_team_id: teams[h].id, away_team_id: teams[a].id,
            matchday: m + 1, home_score: hs, away_score: as, status,
            venue: venue?.name, venue_id: venue?.id,
            referee_id: status !== 'upcoming' ? ids.referee : null,
            scheduled_at: day.toISOString(), match_time: '19:00',
        }).select().single();
        ...
        if (status === 'completed' || status === 'live') {
            ...
            for (let g = 0; g < hs; g++) {
                ...
                await db.from('match_events').insert({
                    match_id: data.id, type: 'goal', team_id: teams[h].id,
                    player_id: scorer.id, player_name: scorer.name, minute: 15 + g * 20, half: g === 0 ? '1st' : '2nd',
                });
            }
            // one yellow card for flavour
            await db.from('match_events').insert({
                match_id: data.id, type: 'yellow_card', team_id: teams[a].id,
                player_id: playersByTeam[teams[a].id][1].id, player_name: playersByTeam[teams[a].id][1].name,
                minute: 55, half: '2nd',
            });
        }
    }
    ...
    const table = {};
    for (const t of teams) table[t.id] = { team_id: t.id, played: 0, won: 0, drawn: 0, lost: 0, goals_for: 0, goals_against: 0, points: 0, form: [] };
    for (const mt of matches.filter((x) => x.status === 'completed')) {
        const H = table[teams[mt.h].id], A = table[teams[mt.a].id];
        H.played++; A.played++; H.goals_for += mt.hs; H.goals_against += mt.as; A.goals_for += mt.as; A.goals_against += mt.hs;
        if (mt.hs > mt.as) { H.won++; A.lost++; H.points += 3; H.form.push('W'); A.form.push('L'); }
        else if (mt.hs < mt.as) { A.won++; H.lost++; A.points += 3; A.form.push('W'); H.form.push('L'); }
    ...
    await db.from('competition_registrations').insert({
        competition_id: comp.id, player_id: lisbonPlayers[6].id, team_id: teams[0].id, organization_id: org.id,
        id_document_type: 'passport', id_document_number: 'P1234567', full_name: 'Pedro Player',
        date_of_birth: '1998-04-12', jersey_number: 7, position: 'ST', status: 'approved',
    });

    const future = new Date(); future.setDate(future.getDate() + 14);
    await db.from('invites').insert([
        { organization_id: org.id, competition_id: comp.id, team_id: teams[1].id, type: 'player_join', email: 'invitee.player@plyaz.test', invited_role: 'player', expires_at: future.toISOString() },
        { organization_id: org.id, competition_id: comp.id, type: 'referee_invite', email: 'invitee.ref@plyaz.test', invited_role: 'referee', expires_at: future.toISOString() },
    ]);

    // Recruitment applications (table may not exist if that migration wasn't applied — tolerate failure)
    try {
        await db.from('teams').update({ is_recruiting_players: true, needed_positions: ['GK', 'CB'], recruitment_message: 'Looking for a keeper and defender.' }).eq('id', teams[2].id);
        await db.from('competitions').update({ is_recruiting_referees: true, recruitment_message: 'Referees wanted for weekend fixtures.' }).eq('id', comp.id);
        await db.from('applications').insert([
            { applicant_id: ids.player, applicant_role: 'player', target_id: teams[2].id, target_type: 'team', status: 'pending', message: 'I would love to join as a defender.' },
            { applicant_id: ids.referee, applicant_role: 'referee', target_id: comp.id, target_type: 'competition', status: 'pending', message: 'Available all weekends.' },
        ]);
    ...
    console.log('Org:', org.name, '| Competition:', comp.name, '| Teams:', teams.length, '| Matches:', matches.length);
    console.log('========================================');
}
